...
def fmeasure_from_file(golden_file, predict_file, label_type="BMES"):
    logger.info(F"Get f measure from file: {golden_file, predict_file}")
    logger.info(F"Label format: {label_type}")
    ...
    logger.info(("Acc:%s, P:%s R:%s, F:%s" % (acc, P, R, F)))
...
def fmeasure_from_singlefile(twolabel_file, label_type="BMES", pred_col=-1):
    ...
    logger.info(("P:%s, R:%s, F:%s" % (P, R, F)))


if __name__ == '__main__':
    # logger.info "sys:",len(sys.argv)
    if len(sys.argv) == 3:
