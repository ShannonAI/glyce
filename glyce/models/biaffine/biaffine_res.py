...
# from dependency_parser.biaffine.my_library.models.WordLanguageModel.embedding_models.model import GlyphEmbedding

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

POS_TO_IGNORE = {'``', "''", ':', ',', '.', 'PU', 'PUNCT', 'SYM'}

@Model.register("biaffine_parser_res")
class BiaffineDependencyParser(Model):
    ...
    def __init__(self,
        ...
        self._head_sentinel = torch.nn.Parameter(torch.randn([1, 1, self.encoder_2.get_output_dim()]))

        self.use_layer_normalization = use_layer_normalization

        if use_layer_normalization:
            self.norm_input = torch.nn.LayerNorm(self.encoder_0.get_input_dim())
            self.norm_hidden = torch.nn.LayerNorm(self.encoder_0.get_output_dim())
    ...
    def forward(self,  # type: ignore
        ...
        """
        # word_list = words['tokens']
        # embedded_text_input = self.glyph(word_list)[0]
        # print(a.shape)
        # input()
        ...
        if self.use_layer_normalization:
            x = self._input_dropout(self.norm_input(embedded_text_input))
            x = self._dropout(self.encoder_0(self.norm_input(x), mask))
            y = x + self._dropout(self.encoder_1(self.norm_hidden(x), mask))
            z = y + self._dropout(self.encoder_2(self.norm_hidden(y), mask))
        else:
            x = self._input_dropout(embedded_text_input)
            x = self._dropout(self.encoder_0(x, mask))
            y = x + self._dropout(self.encoder_1(x, mask))
            z = y + self._dropout(self.encoder_2(x, mask))
